...
class SentdeBot(sc2.BotAI):
    ...
    async def on_step(self, iteration):
        ...
        self.cur_time = (self.state.game_loop/22.4) / 60
    ...
    async def build_scout(self):
        if len(self.units(OBSERVER)) < math.floor(self.cur_time/3):
    ...
    async def expand(self):
        try:
            if self.units(NEXUS).amount < self.cur_time/2 and self.can_afford(NEXUS):
    ...
    async def offensive_force_buildings(self):
        if self.units(PYLON).ready.exists:
            ...
            if self.units(CYBERNETICSCORE).ready.exists:
                if len(self.units(STARGATE)) < self.cur_time:
    ...
    async def attack(self):
        ...
        if len(self.units(VOIDRAY).idle) > 0:
            ...
            target = False
            if self.cur_time > self.do_something_after:
                if self.use_model:
